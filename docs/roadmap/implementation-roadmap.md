...
| **M4 Storefront engine**    | Stores are visible on the web             | Hostname resolution, canonical redirects, status pages, home/product/collection/search/404 routes with a default theme, cart, caching + invalidation, sitemap/robots; query budget test                                                                                                                                   |
| **M5 Page builder**         | Merchants design pages visually           | Document schema v1 + validation + migrations framework, component registry with the initial components, editor shell (dnd, layers, properties, responsive previews, undo/redo, shortcuts), Tiptap rich text, autosave with concurrency, draft/publish/restore                                                             |
