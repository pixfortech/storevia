...
2. **Apps are thin.** A server action or route handler does
   `authenticate → resolve tenant context → validate input → call domain service → map result`. No pricing, permission or entitlement logic in React
   components.
...
| Errors          | Domain functions return typed `Result` values for expected failures (`NOT_FOUND`, `FORBIDDEN`, `LIMIT_REACHED`, `CONFLICT`, `VALIDATION_FAILED`); unexpected errors throw and are caught at the app boundary, logged with the request ID, and shown to the user as a friendly message with that ID. Stack traces never reach clients. |
