...
| MEDIUM | Raw uploads lived under the served asset prefix, weren't bound to the declared size, weren't metered, and could be re-posted                                                       | separate never-served `uploads/{org}/{store}/{media}` prefix; upload target bound to the declared size; POST policy pins `Content-Type: application/octet-stream`; at most 20 uploads per store per hour that didn't finish (deleted or rejected ones included, since their targets stay valid until expiry; tightened in a post-M4 triage check) | media unit (upload keys, policy conditions, object keys refused as upload targets); integration "upload targets" (size and key, pending cap, "deleting or failing an unfinished upload doesn't free its slot") |
...
- **Transfers** take row locks in id order; a theoretical deadlock with a
  ...
  rather than hang.
- **Re-posting to a finished upload's target**: an S3 POST policy can't be
  revoked, so for its 10 minutes a target still accepts bytes after the
  asset is READY or deleted. They land under the unserved `uploads/`
  prefix and aren't metered; production bounds them with the one-day
  lifecycle rule (deployment doc), and the local route doesn't check the
  asset's status (development and test only). A worker sweep of stale
  pending rows and upload keys belongs with the data-lifecycle jobs.
- **Image processing fairness**: the two processing slots per process are
  one FIFO queue shared by every store, so one store completing its 20
  allowed uploads of expensive AVIFs (about 14 s each) can delay other
  stores' image processing in the same process by minutes. The dashboard
  stays responsive; a per-store slot limit or an AVIF pixel cap would fix it.
- **Failed object deletion** after `deleteMedia` commits is logged by key
  and not retried, so a rare storage failure leaves files served after
  their quota was released. The data-lifecycle purge should re-check.
...
Since ADR-0029 the generic parts live in the Site Engine
(`packages/site-engine`) and commerce composes into it; the tests moved with
their code.

| Guarantee                                                | Database (`packages/database/tests/storefront.int.test.ts`)                                                                                                                         | Services, engine and app                                                                                                                                                                                                                                                                                                                 | HTTP (`apps/dashboard/e2e/storefront*.spec.ts`)                                                                                |
| -------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------ |
| Only the host chooses the store                          | `app_storefront_resolve` returns a narrow tuple for ACTIVE domains; reports suspended/archived stores and organisations                                                             | domains unit (normalisation: IDNA, ports, IP literals, URL punctuation) and resolver integration; Site Engine integration: unknown host → site-free 404, secondary host → 301, every path rewritten under the resolved store, a forged `x-sv-store` dropped; Site Engine unit: signed context refused forged, altered, re-keyed or stale | `/sv/{other}/…` → 404; an unknown host → 404; an old address → 301 with path and query                                         |
| Access state                                             | —                                                                                                                                                                                   | Site Engine integration: draft → coming soon, suspended → unavailable, both `noindex`; robots and sitemap follow it                                                                                                                                                                                                                      | a coming-soon store never shows its products                                                                                   |
| Shoppers see sellable data only                          | restrictive policies: ACTIVE products, live variants, non-archived collections, READY media, PUBLISHED versions; column grants (no cost price, no stored HTML); no inventory grants | commerce `storefront.int.test.ts`: DTOs hold public fields only; drafts, archived and other stores' products are not found; Site Engine integration: other stores' pages invisible, a tampered rendition never becomes a URL                                                                                                             | archiving a product makes its page 404 (outbox → worker → cache)                                                               |
...
| Carts never trust the browser                            | cart currency trigger; quantity and line CHECKs; the storefront writes only its store's carts                                                                                       | prices recomputed from current variant prices; 50-line and 99-quantity limits; rate limited per address; token hashed at rest                                                                                                                                                                                                            | add, change and remove re-priced on the server                                                                                 |
| Cart changes can't be forged cross-site                  | —                                                                                                                                                                                   | —                                                                                                                                                                                                                                                                                                                                        | the add-to-cart action posted with a foreign `Origin` is refused and the cart unchanged; the same request from the store works |
| Cookies stay host-only                                   | —                                                                                                                                                                                   | Site Engine unit `boundary.test.ts`: no public code sets a cookie domain; Site Engine integration: the preview cookie has no `Domain`                                                                                                                                                                                                    | the cart cookie is `HttpOnly`, `SameSite=Lax`, without a `Domain`                                                              |
| Published pages are immutable                            | freeze trigger (only PUBLISHED → ARCHIVED); deferred pointer check; one draft and one published version per page                                                                    | the Site Engine's reader loads the published version only                                                                                                                                                                                                                                                                                | —                                                                                                                              |
...
| Every shopper-visible change invalidates the cache       | outbox triggers per source table; services can neither write nor read events; stock emits only when availability can flip                                                           | Site Engine unit (site event → tags, composition order, tag grammar) and commerce unit (catalogue events, composed grammar); worker `outbox.int.test.ts`; Site Engine unit: the tag cache never stores a load that raced an invalidation                                                                                                 | go live and archive reach the storefront within the poll                                                                       |
| Only the worker can invalidate                           | —                                                                                                                                                                                   | storefront unit `revalidate/route.test.ts` (missing, malformed or foreign signatures, stale or future timestamps, a signature over another body → 401, malformed tags ignored); Site Engine integration: unsigned refused, signed drops the tagged entries                                                                               | —                                                                                                                              |
| Preview is store-bound and short-lived                   | —                                                                                                                                                                                   | domains unit (other store, other secret, altered, > 15 minutes); tenancy: `design.edit` needed; Site Engine integration: the token becomes a host-only `HttpOnly` cookie and leaves the URL, a token for another store sets nothing                                                                                                      | the draft store renders with a preview banner                                                                                  |
| Output escaping and structured data                      | —                                                                                                                                                                                   | editor unit (58 tests); Site Engine unit: status pages, sitemaps, JSON-LD and the shell escape every value; canonical URLs refuse anything but site-relative paths                                                                                                                                                                       | CSP with `frame-ancestors 'none'`; JSON-LD `Product`; canonical link                                                           |
| The Site Engine works without commerce (ADR-0029)        | —                                                                                                                                                                                   | Site Engine integration `site-engine.int.test.tsx` (9 tests) with no commerce module loaded: resolution, context, access state, preview, content, media, SEO, shell, invalidation                                                                                                                                                        | the storefront E2E proves commerce composes into it                                                                            |
| Dependency direction (ADR-0029)                          | —                                                                                                                                                                                   | Site Engine unit `boundary.test.ts` walks every module the engine can load, across packages: only generic packages reached, no editor/chart/auth libraries; ESLint forbids direct imports                                                                                                                                                | —                                                                                                                              |
| Query budget (M4-09)                                     | —                                                                                                                                                                                   | `countQueries` harness: home, product, collection and search ≤ 8 queries with site and catalogue reads in one transaction; a product list costs the same for 2 or 40 products                                                                                                                                                            | —                                                                                                                              |
| Store pages ship no application JavaScript (ADR-0029 §4) | —                                                                                                                                                                                   | storefront unit `budget.test.ts`: no `"use client"` module besides the error boundary in the storefront, the Site Engine or the editor                                                                                                                                                                                                   | `storefront-budget.spec.ts`: per-route increment ≤ 8 kB, baseline ≤ recorded + 15%, no editor/chart/auth/admin code loaded     |
...
| Revalidation signature check removed; freshness check removed; tag filter removed                                    | storefront unit `revalidate/route.test.ts` (2, 2 and 1 tests), rerun after the move into the Site Engine                                                             |
| Preview token not bound to its store                                                                                 | domains unit "are refused on another store"                                                                                                                          |
| Signed store context not verified                                                                                    | Site Engine unit "is refused when forged, altered, re-keyed or stale"                                                                                                |
| Pipeline: canonical redirect removed; access state ignored; `/sv/…` paths passed through; preview token not verified | Site Engine integration (1 test each)                                                                                                                                |
| robots.txt ignores access state                                                                                      | Site Engine unit "robots.txt hides everything unless the site is live and not a preview"                                                                             |
| A module the Site Engine loads (media's key grammar) imports `@storevia/entitlements`                                | `boundary.test.ts` "reaches only generic workspace packages" (ESLint alone doesn't see it)                                                                           |
| The cart cookie gets `domain: ".storevia.site"`                                                                      | `boundary.test.ts` "no public code sets a cookie Domain"                                                                                                             |
| `@storevia/domains` imports commerce                                                                                 | ESLint (ADR-0029 rule)                                                                                                                                               |
| A client component validating page documents on the product page                                                     | storefront unit `budget.test.ts`, and `storefront-budget.spec.ts` (product increment 100.8 kB > 8 kB)                                                                |
| `serverActions.allowedOrigins` includes the attacker's origin                                                        | storefront E2E (forged add-to-cart answered 303 instead of being refused)                                                                                            |

Equivalent mutant: removing the reader's early return for a content page
without a handle changes nothing observable (the query then matches
`handle = ''`, which no page has); the early return only saves a query.
...
`CartLine`), deliberately in one place, which is why the database mutants
target it. The commerce tests ("another store's cart token…", "refuses …
other stores' variants") exercise it through the services.

Performance measurements are in
[06-storefront.md §10](./06-storefront.md#10-performance-budget).

## After Milestone 4: local review fixes

| Guarantee                                                         | Where                                                                                                                                                                                                                                                                                                                                                                                                                            |
| ----------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| A database behind the code is refused before anything starts      | database `migrations.int.test.ts` (pending, failed and retried migrations; the migrated test database is up to date; a newer migration on disk shows as pending); `pnpm dev` runs `pnpm db:check`                                                                                                                                                                                                                                |
| Unexpected database errors are diagnosable without logging values | observability unit: a Prisma `P2039` logs its SQLSTATE, constraint and relation names and stack frames, never the message or row values                                                                                                                                                                                                                                                                                          |
| No two queries overlap on one connection (pg@9 readiness)         | database `client.int.test.ts`: the parallel relation loads Prisma issues inside a tenant transaction are serialised on its one connection, which keeps its RLS settings (fails without the serialising client)                                                                                                                                                                                                                   |
| Local media works on whichever host the dashboard is opened       | media unit (same-origin `/media/…` and `/api/media/upload` for the dashboard, absolute URLs for the storefront); dashboard unit `media-urls.test.ts`                                                                                                                                                                                                                                                                             |
| Attached images never silently disappear from the editor          | dashboard unit `media-urls.test.ts`: an asset without a servable rendition stays listed (placeholder, removable)                                                                                                                                                                                                                                                                                                                 |
