...
| `storevia_worker` (M2.5)    | `apps/worker` and `packages/jobs` (ADR-0023)                                    | `BYPASSRLS`               | Job scheduler tables; reads organisation, store and membership ID/status columns; usage counters; `AuditLog` inserts                                                                                                                                                                               |
| `storevia_marketing` (M2.5) | `apps/marketing` only (ADR-0025)                                                | `NOBYPASSRLS`             | `SELECT` on the plan catalogue (`Plan`, `PlanPrice`, `Feature`, `PlanFeature`); its own `marketing:` rows in `RateLimit` through an RLS policy; nothing else                                                                                                                                       |
