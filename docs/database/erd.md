...
| ---------------- | ---------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | -------------------------------------------------------------------------------- |
| **Platform**     | none                         | `Plan`, `PlanPrice`, `PlanPriceProviderRef`, `Feature`, `PlanFeature`, `Theme`, `ThemeVersion`                                                                                                   | Read-only to app role for catalogue tables                                       |
| **Identity**     | `userId`                     | `User`, `Account`, `Session`, `Verification`, `PlatformStaff`, `RateLimit`                                                                                                                       | Accessed only through `packages/auth`; not tenant data                           |
| **Organisation** | `organisationId`             | `Organisation`, `Membership`, `Invitation`, `Subscription`, `SubscriptionEvent`, `Invoice`, `UsageCounter`, `OrganisationFeatureOverride`, `BillingCustomer`, `BillingWebhookEvent`¹, `AuditLog` | `organisationId = app.organisation_id`                                           |
...
erDiagram
  ...
  Store ||--o{ StoreSlugHistory : "formerly"
...
erDiagram
  ...
  Organisation ||--o{ BillingCustomer : "one per provider"
  ...
  PlanPrice ||--o{ PlanPriceProviderRef : "provider price (later)"
```
...
| `Session`          | `token` (HMAC-signed cookie; system-role only, ADR-0021)                              | Cascades from User; expired rows purged daily.                                                                                                                                                             |
| `Verification`     | `identifier` stored hashed (ADR-0021)                                                 | Deleted on use; expired rows purged.                                                                                                                                                                       |
...
| `StoreSlugHistory` | `slug` global                                                                         | A slug the store used before (ADR-0028 §12); never deleted, never reusable by another store.                                                                                                               |

### 4.2 Billing & entitlements

ADR-0009 as refined by [ADR-0022](../adr/0022-provider-neutral-subscriptions-and-entitlements.md)
(provider-neutral sources: `MANUAL`, `MOCK`, `PAYMENT_PROVIDER`).

| Entity                        | Uniqueness                                                                                                                             | Notes                                                                                                                                                                                                                    |
| ----------------------------- | -------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `Plan`                        | `key`                                                                                                                                  | `key` is for seeds/ops; application code never branches on it. Plans are archived (`LEGACY`/`ARCHIVED`), never deleted (subscriptions reference them). Loaded by the reference seed.                                     |
| `PlanPrice`                   | partial unique `(planId, interval, currency) WHERE active`                                                                             | Provider-neutral pricing metadata. A price change = new row, old row `active=false`. Provider price IDs live in `PlanPriceProviderRef` (promoted with the first real adapter).                                           |
| `Feature`                     | `key`                                                                                                                                  | Inserted **by migrations** (keys are the typed `FeatureKey` union; a test asserts parity). `default*` columns are the system default.                                                                                    |
| `PlanFeature`                 | `(planId, featureId)`                                                                                                                  | Complete value: `enabled`, `limit`, `unlimited`, `config`. A trigger validates it against `Feature.type`; "unlimited" is explicit, never `limit NULL`.                                                                   |
| `OrganisationFeatureOverride` | `(organisationId, featureId)`                                                                                                          | Complete replacement value, required `reason`, optional `expiresAt`, `createdById`/`updatedById` (staff). Written only by platform-admin, audited with before/after.                                                     |
| `BillingCustomer`             | `(organisationId, provider)`, `(provider, providerCustomerId)`                                                                         | Maps provider customers to organisations; webhook events are matched to an organisation only through this table.                                                                                                         |
| `Subscription`                | `(provider, providerSubscriptionId)`; at most one non-`EXPIRED` subscription per organisation (partial unique); `(id, organisationId)` | `source` + `CHECK`s tie provider columns to the source and each status to its dates. `providerSyncedAt` = version of the last applied provider snapshot; older snapshots are ignored (out-of-order delivery converges).  |
| `SubscriptionEvent`           | —                                                                                                                                      | Append-only; composite FK `(subscriptionId, organisationId)`. Records source, actor, reason and staff note for every change, whatever its source.                                                                        |
| `BillingWebhookEvent`         | `(provider, providerEventId)`                                                                                                          | The insert **is** the idempotency check. ¹`organisationId` is set once matched; RLS enabled with no app-role grants (platform/system only).                                                                              |
| `UsageCounter`                | `(organisationId, featureId, scopeKey, period)`                                                                                        | Changed with `SELECT … FOR UPDATE` in the same transaction as the counted resource; `CHECK (value >= 0)`; `reconcileUsage` recomputes gauges from source tables and reports drift. Over-limit is computed, never stored. |
...
| `Collection`                | `(storeId, handle)` among non-deleted                                            | `SMART` rules are a validated JSON rule set evaluated by the commerce service (M3 accepts `MANUAL` only). `archivedAt` archives and restores a collection (M3).          |
...
- `Page.publishedVersionId` can only name one of the page's own `PUBLISHED`
  versions, and a `PUBLISHED` version is always the one its page points at
  (deferred constraint trigger, checked at commit; Prisma can't model the
  composite FK `(publishedVersionId, id) → PageVersion(id, pageId)`).
...
| Partial unique: one active price per plan, interval and currency                                   | `PlanPrice`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| CHECK source ↔ provider columns; CHECK each status has its required dates                          | `Subscription` (ADR-0022)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| Trigger: entitlement value matches the feature type (BOOLEAN / LIMIT / CONFIGURATION)              | `PlanFeature`, `OrganisationFeatureOverride`, `Feature` defaults                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| CHECK `value >= 0`                                                                                 | `UsageCounter`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
...
| Deferred constraint trigger: exactly one ACTIVE OWNER per non-deleted organisation at commit       | `Membership`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| Trigger: only the platform may enter/leave `SUSPENDED`                                             | `Store`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| Column-level `UPDATE` grants for `storevia_app` (platform-controlled columns excluded)             | `Organisation`, `Store`, `Membership`, `Invitation`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
...
| Composite FK to a parent-scoped key                                                                | `Page.publishedVersionId` → `PageVersion(id, pageId)` (as a deferred constraint trigger, M4); `NavigationItem.parentId` → `NavigationItem(id, navigationId)` (CASCADE)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |

In the M3 migration the four optional media references (`Store.logoMediaId`,
`Store.faviconMediaId`, `ProductVariant.imageMediaId`,
`Collection.imageMediaId`) and "a variant's option values belong to its own
product" are enforced by **triggers** rather than composite FKs: Prisma can't
model a composite FK that shares a column with another relation and would
report it as drift ([ADR-0027 §1](../adr/0027-commerce-catalogue-and-inventory.md)).
The guarantee is the same, and `packages/database/tests/catalogue.int.test.ts`
covers each one.
